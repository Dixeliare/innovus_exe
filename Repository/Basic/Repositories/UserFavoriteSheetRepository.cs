...
public class UserFavoriteSheetRepository : GenericRepository<user_favorite_sheet>
{
    ...
    {
        return await _context.user_favorite_sheets
            .Include(s => s.sheet_music)
            .Include(u => u.user)
            .ToListAsync();
    }

    public async Task<int> AddAsync(user_favorite_sheet favoriteSheet)
        {
            await _context.user_favorite_sheets.AddAsync(favoriteSheet);
            return await _context.SaveChangesAsync();
        }

        public async Task<int> UpdateAsync(user_favorite_sheet favoriteSheet)
        {
            var existingEntry = await _context.user_favorite_sheets
                                                .FirstOrDefaultAsync(ufs => ufs.user_id == favoriteSheet.user_id &&
                                                                            ufs.sheet_music_id == favoriteSheet.sheet_music_id);

            if (existingEntry == null)
            {
                return 0; // Hoặc throw new NotFoundException("Favorite sheet entry not found.");
            }

            // Cập nhật các thuộc tính nếu có (ví dụ: is_favorite)
            existingEntry.is_favorite = favoriteSheet.is_favorite;

            return await _context.SaveChangesAsync();
        }

        public async Task<int> DeleteAsync(int userId, int sheetMusicId)
        {
        ...
        public async Task<IEnumerable<user_favorite_sheet>> GetUsersWhoFavoritedSheetAsync(int sheetMusicId)
        {
            return await _context.user_favorite_sheets
                                 ...
                                 .Include(ufs => ufs.sheet_music) // Bao gồm thông tin SheetMusic
                                 .AsSplitQuery()
                                 .Where(ufs => ufs.sheet_music_id == sheetMusicId && ufs.is_favorite == true) // Chỉ lấy các mục thực sự là yêu thích
                                 .ToListAsync();
        }
